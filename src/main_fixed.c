    ...
    while (shm->simulation_running) {
        ...
        if (!gang.is_in_prison) {
            ...
            if (mission_planned) {
                ...
                // If not planning a mission, check if prison time is over
                if (gang.prison_time_remaining > 0) {
                    gang.prison_time_remaining--;
                    
                    if (gang.prison_time_remaining <= 0) {
                        gang.is_in_prison = false;
                        log_message("Gang %d released from prison", gang.id);
...
void run_police_process(SimulationConfig config) {
    ...
    while (shm->simulation_running) {
        ...
        process_intelligence_reports(&police, config);
        
        // Take action against gangs
        take_police_action(&police, config, shm, sem_id);
